...
export class TimeService {
  ...
  private readonly MONTH_FORMAT = 'yyyy-MM';
  private readonly TIME_FORMAT = 'HH:mm';
  private readonly TIME_FORMAT_WITH_SECONDS = 'HH:mm:ss';
  private readonly START_TIME_OF_DAY = '00:00';
  private readonly END_TIME_OF_DAY = '23:59';

  private getTimeFormat(timeString: string): string {
    return timeString.length === 5
      ? this.TIME_FORMAT
      : this.TIME_FORMAT_WITH_SECONDS;
  }

  private parseTimeString(timeString: string, timezone?: string) {
  ...
  private parseMonthString(monthString: string, timezone?: string) {
  ...
  private parseIsoDateString(dateString: string, timezone?: string): DateTime {
  ...
  getWeekdayEnum(dateStr: string, zone: string): Weekday {
  ...
  addMinutes(timeString: string, minutes: number, timezone: string) {
    const format = this.getTimeFormat(timeString);
    const parsed = this.parseTimeString(timeString, timezone);
    return parsed.plus({ minutes }).toFormat(format);
  }

  formatTimeInZone(timeString: string, timezone: string) {
    const format = this.getTimeFormat(timeString);
    const parsed = this.parseTimeString(timeString, timezone);
    return parsed.toFormat(format);
  }

  generateIntervalStartTimes(
    startTime: string,
    endTime: string,
    duration: number,
    timezone: string,
  ...
  getMonthStartDate(monthString: string, timezone: string) {
    return this.parseMonthString(monthString, timezone)
      .startOf('month')
      .toISODate();
  }

  getMonthEndDate(monthString: string, timezone: string) {
    return this.parseMonthString(monthString, timezone)
      .endOf('month')
      .toISODate();
  }

  getMonthDatesByWeekday(
    monthString: string,
    weekday: Weekday,
    timezone: string,
  ): string[] {
  ...
  getAllMonthDates(monthString: string, timezone: string): string[] {
    const start = this.parseMonthString(monthString, timezone).startOf('month');
    const end = start.endOf('month');
    const dates: string[] = [];
    let current = start;
  ...
  convertTimeZone(baseTz: string, otherTz: string, time: string) {
